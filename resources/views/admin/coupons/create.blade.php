...
@section('title', __('Add coupon').' — '.config('app.name'))
        ...
        <div class="col-xl-10">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">{{ __('New coupon') }}</h5>
                </div>
                <div class="card-body">
                    <form method="post" action="{{ route('admin.coupons.store') }}" novalidate>
                        @csrf
                        @include('admin.coupons.partials.form-fields', ['coupon' => null, 'hosts' => $hosts, 'gymListings' => $gymListings])
